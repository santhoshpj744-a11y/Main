...
class AttentionLayer(MessagePassing):
    ...
    def __init__(self,
        ...
        head_dim = head_dim if head_dim is not None else hidden_dim // num_heads
        ...
        self.to_g = nn.Linear(head_dim * num_heads +
    ...
    def forward(self,
        ...
        if self.training:
            # instead of dropout in the attention matrix,
            # dropout edges; requires less compute
            mask = torch.rand(
                edge_index.shape[1], device=r.device) > self.dropout
            r = r[mask]
        ...
        x = x + \
            self.attn_postnorm(self._attn_block(x_src, x_dst, r, edge_index))
