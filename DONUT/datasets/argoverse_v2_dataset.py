...
class ArgoverseV2Dataset(Dataset):
    ...
    def download(self) -> None:
        if not os.path.isfile(os.path.join(self.root, f'{self.split}.tar')):
            ...
            request.urlretrieve(self._url, os.path.join(
                self.root, f'{self.split}.tar'))
        ...
        extract_tar(path=os.path.join(
            self.root, f'{self.split}.tar'), folder=self.raw_dir, mode='r')
        ...
        for raw_file_name in self.raw_file_names:
            shutil.move(os.path.join(self.raw_dir, self.split,
                        raw_file_name), self.raw_dir)
