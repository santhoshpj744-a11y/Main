...
class DonutPa(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        # --- CAGrad for K=2 Tasks (Data vs. PINN) ---
        # Use a representative shared layer to estimate the gradient conflict efficiently
        target_param = list(self.proposer.attentions[-1].parameters())[-1]

        # Compute local gradients for the proxy parameter
        ...
        # Default weights (standard average gradient)
        w_data, w_pinn = 0.5, 0.5  

        if grad_data is not None and grad_pinn is not None:
            g1 = grad_data.flatten()
            g2 = grad_pinn.flatten()
            
            g1_sq = torch.dot(g1, g1)
            g2_sq = torch.dot(g2, g2)
            g12 = torch.dot(g1, g2)
            
            diff_sq = g1_sq + g2_sq - 2 * g12
            
            if diff_sq.item() > 1e-8:
                # Step A: Find MGDA optimal weight 'w' (closed form for K=2)
                # This finds the gradient in the convex hull that maximizes the worst-case drop
                w = (g2_sq - g12) / diff_sq
                w = torch.clamp(w, 0.0, 1.0).item()
                
                # Step B: Calculate CAGrad Scaling
                c = 0.4  # CAGrad hyperparameter (usually 0.4 or 0.5)
                
                g0_sq = 0.25 * (g1_sq + g2_sq + 2 * g12)
                g0_norm = torch.sqrt(torch.clamp(g0_sq, min=1e-8)).item()
                diff_norm = torch.sqrt(diff_sq).item()
                
                # Derive exact scalar coefficients for the losses
                gamma = c * (g0_norm / diff_norm) * (1.0 if w >= 0.5 else -1.0)
                
                w_data = 0.5 + gamma
                w_pinn = 0.5 - gamma
                
                # Log cosine similarity
                losses['grad_cosine_similarity'] = (g12 / torch.sqrt(torch.clamp(g1_sq * g2_sq, min=1e-8))).detach()
            else:
                losses['grad_cosine_similarity'] = torch.tensor(1.0, device=target_param.device)
        else:
            losses['grad_cosine_similarity'] = torch.tensor(0.0, device=target_param.device)

        # 2. Formulate final weighted loss
        # Lightning will run the full backward pass automatically on this scalar
        total_loss = (w_data * data_loss) + (w_pinn * pinn_loss)
        ...
        # Log the dynamic CAGrad weights to monitor how they shift during conflict
        self.log('train_cagrad_w_data', w_data, prog_bar=False, on_step=True, on_epoch=True, sync_dist=True, batch_size=batch.num_graphs)
        self.log('train_cagrad_w_pinn', w_pinn, prog_bar=False, on_step=True, on_epoch=True, sync_dist=True, batch_size=batch.num_graphs)
