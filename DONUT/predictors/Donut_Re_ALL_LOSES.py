...
class DonutPa(pl.LightningModule):
    ...
    def forward(self, inp):
        ...
        for layer_pred in pred:
            layer_pred['scale'][0] =torch.clamp( 0.1 + \
                torch.cumsum(layer_pred['scale'][0], dim=2),min=1e-4)
            layer_pred['conc'][0] = 1 / \
                torch.clamp((0.02 + torch.cumsum(layer_pred['conc'][0], dim=2)),min=1e-4)
            if self.over_predict:
                layer_pred['scale'][1] = torch.clamp(0.1 + layer_pred['scale'][1],min=1e-4)
                layer_pred['conc'][1] =1 /torch.clamp( (0.02 + layer_pred['conc'][1]),min=1e-4)
    ...
    def compute_loss(self, data, pred, pi):
        ...
        # ---------------------------------------------------------
        # 1. CALCULATE TOTAL BACKPROP LOSS FIRST
        # ---------------------------------------------------------
        losses['loss'] = sum(v for k, v in losses.items() if k != 'pinn_loss')

        # ---------------------------------------------------------
        # 2. LOG PER-MODE LOSSES (Ignored by the optimizer)
        # ---------------------------------------------------------
        with torch.no_grad():
