...
class Brier(Metric):
    ...
    def update(self,
        ...
        pred, target, prob, valid_mask = valid_filter(
            pred=pred, target=target, prob=prob, valid_mask=valid_mask, keep_invalid_final_step=keep_invalid_final_step)
        pred_topk, prob_topk = topk(self.max_guesses, pred, prob)
        if min_criterion == 'FDE':
            inds_last = (valid_mask * torch.arange(1,
                         valid_mask.size(-1) + 1, device=self.device)).argmax(dim=-1)
            inds_best = torch.norm(pred_topk[torch.arange(pred.size(0)), :, inds_last] -
                                   target[torch.arange(
                                       pred.size(0)), inds_last].unsqueeze(-2),
        ...
        self.sum += (1.0 -
                     prob_topk[torch.arange(pred.size(0)), inds_best]).pow(2).sum()
