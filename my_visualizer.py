...
_FOCAL_AGENT_COLOR: Final[str] = "#ECA25B"  # Orange (History)
...
def _plot_actor_tracks(
    ...
    for track in scenario.tracks:
        ...
        if track.category == TrackCategory.FOCAL_TRACK:
            ...
            # 1. Plot the history (Orange, Solid)
            ...
            # 2. Plot the prediction trail (Red, Dashed)
            # Starts from _OBS_DURATION_TIMESTEPS - 1 so the dashed line connects perfectly to the solid history line
            ...
            # 3. Color the bounding box based on the current timestep
            if timestep < _OBS_DURATION_TIMESTEPS:
                track_color = _FOCAL_AGENT_COLOR
            else:
                track_color = _FOCAL_PRED_COLOR
